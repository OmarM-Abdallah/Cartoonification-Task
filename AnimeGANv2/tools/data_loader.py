...
class ImageGenerator(object):
    ...
    def __init__(self, image_dir,size, batch_size, data_mean, num_cpus = 16):
        ...
        self.data_mean = [13.1360,-8.6698,-4.4661]
    ...
    def read_image(self, img_path1):
        ...
        if 'style' in img_path1.decode() or 'smooth' in img_path1.decode():
            ...
            #print("___",image1[:,:,0],"\n ----",image1[:,:,1],"\n ----",image1[:,:,2])
            #print("___",self.data_mean[2],"\n ----",self.data_mean[1],"\n ----",self.data_mean[0])

            image1[:,:,0] += self.data_mean[2]
            image1[:,:,1] += self.data_mean[1]
            image1[:,:,2] += self.data_mean[0]
            #np.add(image1[:,:,0], self.data_mean[2] , out=image1[:,:,0], casting="unsafe")
            #np.add(image1[:,:,1], self.data_mean[1] , out=image1[:,:,1], casting="unsafe")
            #np.add(image1[:,:,2], self.data_mean[0] , out=image1[:,:,2], casting="unsafe")
